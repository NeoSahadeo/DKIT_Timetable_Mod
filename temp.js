...
var host = "spenterpriselive.dkit.ie:8000"; // host name of the SWS server set July 2021 MR
var startdate = "08/21/2023 00:00:00 GMT" // Change startdate for 2023
var pagehost = "spenterpriselive.dkit.ie:8000"; // this is normally the same as host but since the custom pages can be hosted on a different server need to store in a separate variable set November 2021 MR
...
myFunction("http://" + host + "/reporting/" + style + ";" + fixspace(object) +
                      ";" + idtype + ";" + objectstr + "?t=" + template + "&days=" + days + "&weeks=" + weeks + "&periods=" + periods + "&template=" + template);
}
...
AddGenWeeks(21, 8, 2023, 1, 52, form.elements["weeks"]);
		...
		for (var i=0; i<values.length; i++)
		{
			...
			if (argument == "weeks") {
				...
				if (weekrange.length < 1) {
					...
					var date2temp = date2.getTime();
					var datediff = date2temp - date1temp;
					weekrange = Math.floor(datediff / (1000 * 60 * 60 * 24 * 7))+1;
					if (weekrange < 1) {
						weekrange = 1;
					} else {
						if (weekrange > 52) {
